    ...
    override fun draw(event: Event.LogicalEvent.DrawEvent) {
        event.ensurePreChild {
            if (value) {
                ...
                Pencil.drawSprite(event.drawContext, Vec3i(globalPos.x, globalPos.y, 0), size, onSprite)
            } else {
                RenderSystem.setShaderTexture(0, offSprite.atlasId)
                Pencil.drawSprite(event.drawContext, Vec3i(globalPos.x, globalPos.y, 0), size, offSprite)
            }
        }
    }
