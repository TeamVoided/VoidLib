package org.teamvoided.voidlib.vui.node.screen
...
abstract class VuiScreen(title: Text): Screen(title) {
    ...
    protected open var initCallback: (() -> Unit)? = null
    ...
    override fun render(matrices: MatrixStack, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        parent.update(UpdateContext(delta, Vec2i(mouseX, mouseY)))
        parent.draw(DrawContext(matrices, Vec2i(mouseX, mouseY), partialTicks, delta, false))
        parent.draw(DrawContext(matrices, Vec2i(mouseX, mouseY), partialTicks, delta, true))
