...
import org.teamvoided.voidlib.vui.node.NodeIds.STRING_INPUT

open class StringInputNode(override var name: String): Node() {
    ...
    override var onKeyPress: ((InputEvent.KeyPressEvent) -> Unit)? = {
        if (captureText) {
            if (it.keyCode == GLFW.GLFW_KEY_BACKSLASH) {
                ...
                text.replace(0, text.length, "$t1$t2")
            }
            ...
            if (it.keyCode == GLFW.GLFW_KEY_RIGHT) {
                if (textPosition < text.length) textPosition += 1
            }
        }
    }
    ...
    override var onCharTyped: ((InputEvent.CharTypedEvent) -> Unit)? = {
        ...
        val t2 = text.safeSubstring(textPosition + 1)
        if (captureText) text.replace(0, text.length, "$t1$t2")
    }
    ...
    override fun typeId(): Identifier = STRING_INPUT
}